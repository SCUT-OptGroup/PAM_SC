...
%  
%  where f(X) is Laplacian noise /Gaussian noise / Logistic noise
%  
% in the order: U--->V---->U
%
%% **********************************************************************
...
function [iter,obj,time_list,obj_list,diffX_list]  = PALM_Irate(Mstar,U,V,y,ybar,f,fprime,spidx,OPTIONS,pars,lambda,mu)
...
if  (printyes)
    fprintf('\n *****************************************************');
    fprintf('******************************************');
    fprintf('\n ************** PALM  for  solving low-rank recovery  problems  ********************');
    fprintf('\n ****************************************************');
    fprintf('*******************************************');
...
time_list = zeros(maxiter,1);    obj_list = zeros(maxiter,1);    
   
diffX_list = zeros(maxiter,1);   relerr_list = zeros(maxiter,1);
...
[Loss,gradU] = funU(V,y,ybar,spidx,f,fprime,nr,nc,Xold);
...
for iter = 1:maxiter  
 ...
 %% ****************** to search the Lip-constant *******************
    ...
    Loss_Unew = funU(V,y,ybar,spidx,f,fprime,nr,nc,UnewVt);
  ...
  %% ****************** to compute Vnew **************************
    if iter==1       
        ...
        [Loss,gradV] = funV(Unew,y,ybar,spidx,f,fprime,nr,nc,UnewVt);
        
    else
        [~,gradVold] = funV(Unew,y,ybar,spidx,f,fprime,nr,nc,Unew*Vold');
 ...
 %% ****************** to search the Lip-constant  *******************
    ...
    Loss_Vnew =  funV(Unew,y,ybar,spidx,f,fprime,nr,nc,Xnew);
...
%% ******************** Optimality checking ************************
    ...
    [Loss,gradUnew] = funUV(Unew,Vnew,y,ybar,spidx,f,fprime,nr,nc,Xnew);
    ...
    diffX_list(iter) = measure;

    relerr = norm(Xnew-Mstar,'fro')/norm(Mstar,'fro');
          
    relerr_list(iter) = relerr;
   ...
   %% ******* BB step for estimating the Lip-constant of grad_U F(U,V) ************   
    ...
    [~,gradU] = funU(Vnew,y,ybar,spidx,f,fprime,nr,nc,U*Vnew');
